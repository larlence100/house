...
<body >
	...
	<nav class="breadcrumb hidden">
		...
		<span class="c-gray en">&gt;</span> 学区管理
	...
	<div class="page-container">
		<table class="table table-border table-bordered table-hover table-bg">
			<thead>
				<tr>
					<th scope="col" colspan="8">
						<a class="btn btn-primary radius" href="javascript:;" onclick="admin_xuequ_add('添加学区','<?php echo U('Ziyuan/addXuequ');?>','500','400')">
							<i class="Hui-iconfont">&#xe600;</i> 添加学区
						</a>
			...
			<tbody id="xsjxueq">
				<?php if(is_array($xuequ)): $i = 0; $__LIST__ = $xuequ;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$xq): $mod = ($i % 2 );++$i;?><tr class="text-c">
						<td><?php echo ($i+$firstRow); ?></td>
						<td><?php echo ($xq["xuexiaom"]); ?></td>
						<td><?php echo ($xq["pinyinjs"]); ?></td>
						<td><?php echo ($xq["xuexiaodz"]); ?></td>
						<td><?php echo ($xq["duikou"]); ?></td>
						<td class="f-14">
							<a title="编辑" href="javascript:;" onclick="admin_xuequ_edit('编辑学区','<?php echo U('Ziyuan/editXuequ',array('id'=>$xq['id']));?>','500','400')" style="text-decoration:none" >
								<i class="Hui-iconfont" style="font-size: 18px;">&#xe6df;</i>
							</a>
							<a title="删除" href="javascript:;" onclick="admin_xuequ_del(this,'<?php echo ($xq["id"]); ?>')" class="ml-5" style="text-decoration:none">
...
<script>
	...
	<script type="text/javascript">
		/*管理员-配置-添加*/
		function admin_xuequ_add(title,url,w,h){
			layer_show(title,url,w,h);
		}
		/*管理员-配置-编辑*/
		function admin_xuequ_edit(title,url,w,h){
			layer_show(title,url,w,h);
		}
		/*管理员-配置-删除*/
		function admin_xuequ_del(obj,id){
			layer.confirm('配置删除须谨慎，确认要删除吗？',function(index){
				$.ajax({
					type: 'POST',
					url: '<?php echo U('Ziyuan/delXuequ');?>',
					data: {'id':id},
					dataType: 'json',
		...
		function xsjxueq(){
			$.ajax({
				url:'<?php echo U('Ziyuan/get_xueqxsj');?>',
				Type:"POST",
				dataType:"json",
				success:function(data){
					var pics = data.pics;
					var sj='<tr class="text-c"><td style="color:red">new</td><td>'+pics["xuexiaom"]+'</td><td>'+pics["pinyinjs"]+'</td><td>'+pics["xuexiaodz"]+'</td><td>'+pics["duikou"]+'</td><td></td></tr>';
					$("#xsjxueq").prepend(sj);
				}
			});
		};
